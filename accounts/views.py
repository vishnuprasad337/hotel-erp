...
from hotel.models import Room, Staff
from pms.models import Booking, Room as PmsRoom, RoomUnit

from .forms import HotelForm
from .models import (
    Amenity, Department, Hotel, HotelModule, Permission, RolePermission
)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                redirect_url = f"https://{domain.domain}" if settings.IS_RENDER else f"http://{domain.domain}{settings.PORT}"
...
def get_amenities(request):
    try:
        default_amenities = list(
            Amenity.objects.filter(amenity_type="default").values("id", "name", "description")
        )
        premium_amenities = list(
            Amenity.objects.filter(amenity_type="premium").values("id", "name", "description")
        )
        return JsonResponse({"default": default_amenities, "premium": premium_amenities}, status=200)
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
...
def save_selected_amenities(request):
    ...
    return JsonResponse({"status": "success", "saved_count": created_count, "hotel": hotel.hotel_name})
...
def delete_department(request, dept_id):
    try:
        ...
        dept.delete()
        return JsonResponse({"message": "Department deleted", "id": dept_id})
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
...
def assign_permission(request):
    if request.method == "POST":
        ...
        RolePermission.objects.bulk_create([
            RolePermission(role=department, permission=p) for p in permissions_qs
        ])
...
def staff_login(request):
    if request.method == "POST":
        ...
        with schema_context(current_tenant.schema_name):
            staff = (
                Staff.objects.select_related("department", "hotel")
